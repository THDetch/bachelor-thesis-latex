...
To further assess the individual contribution of each loss component, an ablation study was conducted. The model was trained under different configurations, including combinations of the individual losses and the full combined objective. Quantitative and qualitative results of this ablation are presented and discussed in Section~\ref{sec:ablation_loss}.



--SAR-to-Optical Image Translation Using Vision Transformer-Based CGAN
why convential losses strugle, use a perceptual loss instead


Adversarial loss, reconstruction loss (L1/L2), perceptual loss, spectral metrics (e.g., SAM).



-- Cloud Removal in Remote Sensing Images Using Generative Adversarial Networks and SAR-to-Optical Image Translation

\textcolor{red}{Our results also showed that adding the SSIM loss function to the classical pix2pix loss Effect of SSIM Loss Function: To demonstrate the impact of the SSIM loss function, we abandoned it in different modes. Table VII presents the results of our investigation. The results of the evaluation metrics improved by adding the SSIM loss function. The use of SSIM with L1 in both GANs scores the best results for all metrics.}
...
\begin{algorithm}[H]
\caption{Computation of Sentinel-2 Value Ranges}
\begin{algorithmic}[1]
\State \textbf{Input:} SEN12MS dataset (winter subset)
\State \textbf{Output:} Minimum and maximum values per band for representative tiles
\State
\State Select the \texttt{winter} subset from SEN12MS
\For{each Region of Interest (ROI) in the winter subset}
    \State Randomly select one tile from the ROI
    \State Load the corresponding Sentinel-2 image
    \For{each spectral band $b$ in the image}
        \State Compute $\min(b)$ and $\max(b)$ over all pixels
    \EndFor
\EndFor
\State Report the per-band minimum and maximum values for each selected tile
\end{algorithmic}
\end{algorithm}
...
\bottomrule
\end{tabular}
\caption{Frequency of common evaluation metrics used in SAR-to-optical and cloud-removal studies.}
\label{tab:iqa}
\end{table}
